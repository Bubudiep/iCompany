...
const Details_operator = ({ op_id }) => {
  ...
  const handleDelete = () => {
    if (op_id && user?.info?.isSuperAdmin) {
      Modal.confirm({
        title: "Xác nhận xóa người lao động?",
        content:
          "Bạn có chắc chắn muốn xóa người lao động này khỏi hệ thống không?",
        onOk: () => {
          setLoading(true);
          api
            .delete(`/ops/${op_id}/`, user.token)
            .then((res) => {
              setOp({});
            })
            .catch((e) => {
        ...
        op?.id ? (
          ...
          <div className="flex flex-1 p-1 flex-col">
            ...
            <div className="flex justify-center flex-1 items-center mb-32">
              <Empty description="Không thấy người lao động này nữa!" />
            </div>
          </div>
        )
      ) : (
...
export default Details_operator;
